...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \emph{Exact trace} for moderate $D$: compute $\divg v = \tr(\partial v/\partial x)$ efficiently using vectorization. In PyTorch, we employ \texttt{torch.vmap} combined with \texttt{torch.func.jacrev} to compute per-sample Jacobians in parallel, avoiding the naive $O(D)$ loop. This makes exact evaluation feasible for dimensions up to $D \approx 100$.
    ...
    \end{enumerate}
\end{itemize}
Cache $(\log \tilde q(x), \log q(x), \log \pi(x))$ at the current state. Additionally, to ensure numerical stability during the Stage 2 correction, we explicitly clamp the log-acceptance ratio $\log \alpha_2 = \min(0, \log r_2)$. This prevents floating-point errors (where $\log \tilde q$ underestimates slightly) from resulting in $\alpha_2 > 1$, which would violate detailed balance.
...
For the velocity network $v_\phi(x,t)$, we employ a \texttt{TimeResNet} architecture: a residual network where time $t$ is embedded via Gaussian Fourier projections \citep{tancik2020fourier} and injected into each block via adaptive Group Normalization. This architecture provides significantly better expressivity for learning complex, multimodal transport maps compared to simple concantenation-based MLPs.
