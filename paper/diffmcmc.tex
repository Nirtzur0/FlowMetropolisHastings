...
\begin{enumerate}
    ...
    \item \textbf{Strategy}: We employed a hybrid **Gradient-Based Initialization** strategy. First, a coarse global search ($N=5000$) identifies the basin of attraction. Then, we leverage the differentiability of our TMM solver to run **Adam Optimization** for 200 steps, minimizing the negative log-likelihood to precisely pinpoint the local mode before starting the DiffMCMC chain.
\end{enumerate}

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dataset_stats.png}
    \caption{Dataset Characteristics. Left: Visualization of the 2,000 reflectance spectra in the dataset, showing significant diversity and interference patterns. Right: Uniform distribution of layer thicknesses across the generated hypotheses.}
    \label{fig:dataset}
\end{figure}
...
\textbf{Results}: The DiffMCMC framework successfully integrated the differentiable TMM physics engine. By combining gradient-based initialization with the rigorous local-global mixing of DiffMCMC, we achieved robust spectral reconstruction (Fig. \ref{fig:photonic}). The ability to differentiate through the physics engine provided a crucial advantage in initializing the chain.

\subsection{Comparison to Classical Global Optimization}
To interpret the inferred results and distinguish between algorithmic convergence and physical degeneracy, we benchmarked DiffMCMC against **Differential Evolution (DE)** (Scipy implementation), a gold-standard derivative-free global optimizer.
\begin{table}[H]
\centering
\begin{tabular}{l c c c}
\toprule
Method & Time (s) & Spectral MSE ($10^{-3}$) & Parameter RMSE (nm) \\
\midrule
Diff. Evolution (DE) & 25.8 & \textbf{2.26} & 67.4 \\
DiffMCMC (Ours) & \textbf{11.2} & 8.58 & 118.6 \\
\bottomrule
\end{tabular}
\caption{Benchmark on 10-layer Photonic Inference. Expected Noise MSE $\approx 10.0$ ($10^{-3}$).}
\label{tab:classical}
\end{table}

\textbf{Analysis of Degeneracy}: Both methods achieved a Spectral MSE below the noise floor ($\sigma^2 \approx 0.01$), indicating successful convergence to a valid likelihood mode. However, both methods exhibit high Parameter RMSE relative to the domain size ($[50, 300]$nm). This confirms the problem is **physically degenerate**: multiple structural configurations map to indistinguishable spectra.
\textbf{Advantage}: DiffMCMC converged \textbf{2.3x faster} than the classical baseline while providing uncertainty quantification (posterior samples) rather than a single point estimate.
